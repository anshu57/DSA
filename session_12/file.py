...
#max number of moves
def maximumMoves(sr, sc, er, ec, vis, ans):
    if sr == er and sc == ec:
        return 0
    elif vis[sr][sc] == True:
        return 0
    ...
    # for right movement
    if sr + 0 <= er and sc + 1 <= ec and vis[sr + 0][sc + 1] == False:
        moves = max(moves,maximumMoves(sr + 0, sc + 1, er, ec, vis, ans + 'r')+1)

    

    # for down movement
    if sr + 1 <= er and sc + 0 <= ec and vis[sr + 1][sc + 0] == False:
        moves = max(moves,maximumMoves(sr + 1, sc + 0, er, ec, vis, ans + 'd')+1)
    
    # for left movement
    if sr + 0 <= er and sc - 1 >= 0 and vis[sr + 0][sc - 1] == False:
        moves = max(moves,maximumMoves(sr + 0, sc - 1, er, ec, vis, ans + 'l')+1)

    # for up movement
    if sr - 1 >= 0 and sc + 0 >= 0 and vis[sr - 1][sc + 0] == False:
        moves = max(moves,maximumMoves(sr - 1, sc + 0, er, ec, vis, ans + 'u')+1)
